...
final class StandingsController
{
    ...
    public function index(Request $request): Response
    {
        ...
        $sql = 'SELECT * FROM touchbase_standings WHERE 1=1';
        $params = [];
    ...
    public function headToHead(Request $request): Response
    {
        ...
        $sql = 'SELECT m.*,
                ...
                JOIN touchbase_teams t1 ON (t1.id = ? OR t1.id = ?)
                JOIN touchbase_teams t2 ON (t2.id = ? OR t2.id = ?)
        ...
        if ($seasonId) {
            $sql .= ' AND m.tournament_id IN (SELECT id FROM touchbase_tournaments WHERE season_id = ?)';
            $params[] = (int) $seasonId;
        }
